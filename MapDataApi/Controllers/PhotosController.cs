...
namespace MapDataApi.Controllers
{
    ...
    {
        private readonly IMapUserRepository _repo;
        private readonly IMapper _mapper;
        private readonly IOptions<CloudinarySettings> _cloudinaryConfig;
        private Cloudinary _cloudinary;
        public PhotosController(IMapUserRepository repo, IMapper mapper,
        ...
        {
            if (userId != int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value))
            return Unauthorized("Not the user");

            var user = await _repo.GetUser(userId);
            
            if(!user.Photos.Any(p => p.Id == id))
            return Unauthorized("No matching photo");
